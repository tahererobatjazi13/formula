...
import ir.kitgroup.formulaNew.adapter.ProductUsageDetailAdapter
...
import ir.kitgroup.formulaNew.databinding.FragmentProductUsageDetailsBinding
...
class ProductUsageDetailsFragment : Fragment() {
    ...
    private fun generatePdfWithData(context: Context, items: List<PdfRowData>) {
        try {
            ...
            val farsiFontSmall = Font(baseFont, 13f, Font.NORMAL)
            ...
            val symbolBaseFont = BaseFont.createFont(
                "assets/fonts/DejaVuSans.ttf",
                BaseFont.IDENTITY_H,
                BaseFont.EMBEDDED
            )
            val symbolFont = Font(symbolBaseFont, 16f, Font.NORMAL)
            ...
            val columnWidths = floatArrayOf(3f, 2f, 2f, 1f)
            table.setWidths(columnWidths)

            // اضافه کردن هدر جدول
            table.addCell(
                createCell(
                    context.getString(R.string.label_choose),
                    farsiFontSmall,
                    headerColorBase
                )
            )
            ...
            for (i in items.indices) {
                ...
                table.addCell(createCell(checkSymbol, symbolFont, rowColor))
