package ir.kitgroup.formulaNew.viewmodel
...
import ir.kitgroup.formulaNew.database.AppDatabase
...
class MaterialViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun canDeleteMaterial(materialId: Int, onResult: (Boolean) -> Unit) {
        viewModelScope.launch {
            val count = materialDao.getMaterialUsageCount(materialId)
            onResult(count == 0)
        }
    }
    fun canDeletePackaging(packagingId: Int, onResult: (Boolean) -> Unit) {
        viewModelScope.launch {
            val count = materialDao.getPackagingUsageCount(packagingId)
            onResult(count == 0)
        }
    }
